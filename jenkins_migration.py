...
def backup_jenkins_data(namespace, pod, backup_path):
    ...
    '''
    # Check if the jenkins_config directory exists before attempting to back it up
    print(f"Checking if {config_backup_path} exists in {namespace} namespace...")
    check_command = f"kubectl -n {namespace} exec {pod} -- test -d {config_backup_path}"
    try:
        run_command(check_command)
        print(f"{config_backup_path} exists. Backing it up...")
        run_command(f"kubectl -n {namespace} exec {pod} -- tar -czvf /tmp/jenkins-config-backup.tar.gz {config_backup_path}")
    except subprocess.CalledProcessError:
        print(f"{config_backup_path} does not exist. Skipping config backup.")
    '''
...
def restore_jenkins_data(namespace, pod, backup_path, config_backup_path):
    ...
    '''
    try:
        run_command(f"kubectl -n {namespace} exec {pod} -- tar -xzvf /tmp/jenkins-config-backup.tar.gz --strip-components=1 -C {config_backup_path}")
    except subprocess.CalledProcessError:
...
def main():
    ...
    backup_jenkins_data(SOURCE_NAMESPACE, SOURCE_POD_NAME, "/var/jenkins_home")
    ...
    restore_jenkins_data(TARGET_NAMESPACE, TARGET_POD_NAME, "/var/jenkins_home")
