...
def backup_jenkins_data(namespace, pod, container, backup_path):
    print(f"Backing up Jenkins data from {namespace} namespace...")
    run_command(f"kubectl -n {namespace} exec {pod} -c {container} -- tar -czvf /tmp/jenkins-backup.tar.gz {backup_path}")

def copy_backups_to_local(namespace, pod, container):
    print("Copying backups to local machine... This might take a while.")
    run_command(f"kubectl -n {namespace} cp {pod}:/tmp/jenkins-backup.tar.gz ./jenkins-backup.tar.gz -c {container}")

def copy_backups_to_pod(namespace, pod, container):
    print(f"Copying backups to new Jenkins pod in {namespace} namespace... This might take a while.")
    run_command(f"kubectl -n {namespace} cp ./jenkins-backup.tar.gz {pod}:/tmp/jenkins-backup.tar.gz -c {container}")

def restore_jenkins_data(namespace, pod, container, backup_path):
    print(f"Restoring backups in {namespace} namespace... This might take a while.")
    run_command(f"kubectl -n {namespace} exec {pod} -c {container} -- tar -xzvf /tmp/jenkins-backup.tar.gz --strip-components=2 -C {backup_path}")
...
def main():
    ...
    backup_jenkins_data(SOURCE_NAMESPACE, POD_NAME, CONTAINER_NAME, "/var/jenkins_home")
    copy_backups_to_local(SOURCE_NAMESPACE, POD_NAME, CONTAINER_NAME)

    if SWITCH_TENANT:
        switch_azure_ad(TARGET_TENANT_ID)

    switch_kube_context(TARGET_CONTEXT)
    copy_backups_to_pod(TARGET_NAMESPACE, POD_NAME, CONTAINER_NAME)
    restore_jenkins_data(TARGET_NAMESPACE, POD_NAME, CONTAINER_NAME, "/var/jenkins_home")
